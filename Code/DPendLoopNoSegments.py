...
@np.vectorize
def doublePendTest(theta1, theta2, l, m, g):
    # Define starting values
    y0 = [theta1, theta2, 0.0, 0.0]
    t = np.linspace(0, Tmax, Tnumber)
    sol = odeint(doublePendODE, y0, t, args=(l, m, g), mxstep=5000000)
...
lengthsToTest = [10, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000]
...
def imageOutput(length):
    ...
    X,Y = np.meshgrid(x,y)
    Z = doublePendTest(X,Y, length, m, g)
