...
# Automate whole process + output a lot of graphs on raspberry pi over time (will it run if ssh terminal session is closed)
# Try different colourmaps
# Output time as minutes and seconds rather than just seconds
# Come up with better name for theta1 and theta2 (theta and phi?)
# Check how much faster JIT compiler improves speed + include in essay
# Check return value on non-flip outcome
# Write overall structure for summer essay with rerferences to contacting author of wiki article
# Go through full derivations of Lagrangian equations and partial derivatives
# ADD COMMAND LINE ARGUMENTS AND DEFAULTS FOR ALL VARIABLES
# Work out way to upload to joshgreensmith.ml
